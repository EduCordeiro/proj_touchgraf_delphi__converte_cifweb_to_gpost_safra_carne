...
uses
  ...
  Dialogs, StdCtrls,  Classes, SysUtils, Forms, ComObj,
...
begin
  ...
  try
    ...
    begin
      ...
      objParametrosDeEntrada.PESO_FOLHA                                 := objArquivoDeConfiguracao.getConfiguracao('PESO_FOLHA');
      objParametrosDeEntrada.PESO_ENVELOPE                              := objArquivoDeConfiguracao.getConfiguracao('PESO_ENVELOPE');
      ...
      objParametrosDeEntrada.PESO_CAPA_COTRA_CAPA                       := objArquivoDeConfiguracao.getConfiguracao('PESO_CAPA_COTRA_CAPA');
      objParametrosDeEntrada.PESO_LAMINA_MENSAGENS                      := objArquivoDeConfiguracao.getConfiguracao('PESO_LAMINA_MENSAGENS');
...
var
  ...
  sEXTENCAO_ARQUIVOS                : string;
  sMASCARA_ARQUIVOS                 : string;
  ...
  iPesoLaminaMensagens              : Integer;
  iPesoCapaContraCapa               : Integer;
  ...
  stlListaRetencaoLinha             : TStringList;
...
begin
  ...
  stlListaRetencaoLinha            := TStringList.create();
  ...
  sPathMovimentoRelatorio          := objString.AjustaPath(objParametrosDeEntrada.PATHSAIDA) + 'RELATORIO'  + PathDelim;
...
//  ForceDirectories(sPathmovimentoBackupZip);
  ...
  begin
    ...
    AssignFile(Arq_Arquivo_Entada, objString.AjustaPath(objParametrosDeEntrada.PathEntrada) + 'RETENCAO.TXT');
    reset(Arq_Arquivo_Entada);
    ...
    Begin
      ...
      sChaveRetencao := Copy(sLinha, 1, 16);

      stlListaRetencao.Add(sChaveRetencao);

      sLinha := objString.StringReplaceList(sLinha, objParametrosDeEntrada.lista_de_caracteres_invalidos);

      stlListaRetencaoLinha.Add(sLinha);
  ...
  begin
    ...
    //=====================================================================================================================================================================
    // TRATANDO A MÁSCARA E A EXTENÇÃO PARA SUBSTITUIÇÃO NA CRUIAÇÃO DO NOVO ARQUIVO
    //=====================================================================================================================================================================
    sMASCARA_ARQUIVOS  := objParametrosDeEntrada.EXTENCAO_ARQUIVOS;
    sEXTENCAO_ARQUIVOS := Copy(sMASCARA_ARQUIVOS, LastDelimiter('.', sMASCARA_ARQUIVOS), (length(sMASCARA_ARQUIVOS) -  LastDelimiter('.', sMASCARA_ARQUIVOS)) + 1);

    if trim(sEXTENCAO_ARQUIVOS) = '' then
      sEXTENCAO_ARQUIVOS := '.txt';

    sArquivoSaida := StringReplace(sArquivoEntrada, sEXTENCAO_ARQUIVOS, '.GPOST', [rfReplaceAll, rfIgnoreCase]) + sEXTENCAO_ARQUIVOS;
    //=====================================================================================================================================================================

    //==============================================================================
    //  DEFINIÇÃO DO PESO DA CAPA E CONTRA CAPA
    //==============================================================================
    iPesoCapaContraCapa := StrToInt(objParametrosDeEntrada.PESO_CAPA_COTRA_CAPA);
    //==============================================================================


    //==============================================================================
    //  DEFINIÇÃO DO PESO DAS LAMINAS MENSAGENS
    //==============================================================================
    iPesoLaminaMensagens := 0;

    if pos('_D_', sArquivoEntrada) > 0 then
      iPesoLaminaMensagens := StrToInt(objParametrosDeEntrada.PESO_LAMINA_MENSAGENS) * 2;

    if (pos('_N_', sArquivoEntrada) > 0) OR ((pos('_F_', sArquivoEntrada) > 0)) then
      iPesoLaminaMensagens := StrToInt(objParametrosDeEntrada.PESO_LAMINA_MENSAGENS) * 1;
    //==============================================================================

    AssignFile(Arq_Arquivo_Entada, objString.AjustaPath(objParametrosDeEntrada.PathEntrada) + sArquivoEntrada);
    reset(Arq_Arquivo_Entada);

    AssignFile(Arq_Arquivo_Saida, sPathMovimentoArquivos + sArquivoSaida) ;
    Rewrite(Arq_Arquivo_Saida);
    ...
    Begin
      ...
      sChaveRetencao                    := copy(sLinha, 20, 16);

      sCIF_GPOST                        := '';

      sCEP                              := copy(sLinha, 218, 08);
      ...
      //===============================================================
      //  utilizando o sCepStatus para realizar a retenção por código.
      //===============================================================
      if stlListaRetencao.IndexOf(sChaveRetencao) >= 0 then
        sCepStatus := '05';
      //===============================================================
      ...
      sCIF_WEB                            := copy(sLinha, 1899, 34);

      if copy(sCIF_WEB, 1, 2) = objParametrosDeEntrada.DR_POSTAGEM then
      begin

        sNomeClienteEndereco              := copy(sLinha, 101, 30);

        sFolhas                           := copy(sLinha, 36, 3);
        iFolhas                           := StrToInt(sFolhas);

        iTotalPeso                        := (iFolhas * StrToInt(objParametrosDeEntrada.PESO_FOLHA)) // PESO FOLHA
                                           + StrToInt(objParametrosDeEntrada.PESO_ENVELOPE)          // PESO ENVELOPE
                                           + iPesoLaminaMensagens                                    // PESO LAMINA MENSAGENS
                                           + iPesoCapaContraCapa;                                    // PESO CAPA CONTRA CAPA
        ...
        //====================================================================================
          ...
          //=================================================================================================================================================================
            sComando := 'insert into ' + objParametrosDeEntrada.TABELA_PROCESSAMENTO
                      ...
                      + '","' + objParametrosDeEntrada.CODIGO_AVALIACAO_TECNICA
                      + '","' + sCicloPostagem
                      + '","' + sAnoPostagem
  ...
  stlRelatorioObjRetidosAnalitico.Add('LOTE  SEQUENCIA   ARQUIVO ORIGEM                                    NOME                           CHAVE RETENCAO   CEP');
  ...
  //====================================================================================
   ...
   Begin
     ...
     if StrToBoolDef(objParametrosDeEntrada.USAR_PATH_PERSONALIZADO_CIF, false) then
       sPathmovimentoCIF                := objString.AjustaPath(objParametrosDeEntrada.PATH_DEFAULT_ARQUIVOS_SAIDA_CIF)
     ...
     Begin
       ...
       sLinha := __queryMySQL_processamento2__.FieldByName('SEQUENCIA_OBJETO').AsString
         + '|' + FormatFloat('000000', __queryMySQL_processamento2__.FieldByName('PESO').AsInteger)
     ...
     //========================================================================================
     sComando := 'SELECT  NUMERO_LOTE, SEQUENCIA_OBJETO, ARQUIVO, NOME_CLIENTE_ENDERECO, CHAVE_RETENCAO, CEP_DESTINO, CEP_STATUS FROM ' + objParametrosDeEntrada.TABELA_PROCESSAMENTO
               + ' WHERE NUMERO_CARTAO = "' + sNumeroCartao + '"'
               + '   AND NUMERO_LOTE   = "' + sLoteFAC + '"'
               + '   AND mid(CIF_WEB, 1, 2) = 72 '
               + '   AND CEP_STATUS    <> "01" '
     ...
     objConexao.Executar_SQL(__queryMySQL_processamento2__, sComando, 2);
      ...
      //planilha.caption := 'Teste Excel';
      //planilha.visible := False;

      //iContLinasPlanilha := 1;

      //planilha.cells[iContLinasPlanilha, 1] := 'NUMERO_LOTE';
      //planilha.cells[iContLinasPlanilha, 2] := 'SEQUENCIA_OBJETO';
      //planilha.cells[iContLinasPlanilha, 3] := 'ARQUIVO';
      //planilha.cells[iContLinasPlanilha, 4] := 'NOME_CLIENTE_ENDERECO';
      //planilha.cells[iContLinasPlanilha, 5] := 'CHAVE_RETENÇÃO';
      //planilha.cells[iContLinasPlanilha, 6] := 'CEP_DESTINO';
      //planilha.cells[iContLinasPlanilha, 7] := 'MOTIVO';

     while not __queryMySQL_processamento2__.Eof do
     Begin

       //iContLinasPlanilha := iContLinasPlanilha + 1;

       sChaveRetencao := __queryMySQL_processamento2__.FieldByName('CHAVE_RETENCAO').AsString;
       if __queryMySQL_processamento2__.FieldByName('CEP_STATUS').AsString = '05' then
         sLabelMotivoRetencao := 'RETIDO POR CHAVE DE RETEÇÃO'
       ELSE
       BEGIN
       ...
       stlRelatorioObjRetidosAnalitico.Add(
                ...
                + ' ' + objString.AjustaStr(sChaveRetencao, 16)
                + ' ' + objString.AjustaStr(__queryMySQL_processamento2__.FieldByName('CEP_DESTINO').AsString, 8)
                + ' ' + sLabelMotivoRetencao
                );

      //planilha.cells[iContLinasPlanilha, 1] := __queryMySQL_processamento2__.FieldByName('NUMERO_LOTE').AsString;
      //planilha.cells[iContLinasPlanilha, 2] := __queryMySQL_processamento2__.FieldByName('SEQUENCIA_OBJETO').AsString;
      //planilha.cells[iContLinasPlanilha, 3] := objString.AjustaStr(__queryMySQL_processamento2__.FieldByName('ARQUIVO').AsString, 49);
      //planilha.cells[iContLinasPlanilha, 4] := objString.AjustaStr(__queryMySQL_processamento2__.FieldByName('NOME_CLIENTE_ENDERECO').AsString, 30);
      //planilha.cells[iContLinasPlanilha, 5] := objString.AjustaStr(sChaveRetencao, 16);
      //planilha.cells[iContLinasPlanilha, 6] := objString.AjustaStr(__queryMySQL_processamento2__.FieldByName('CEP_DESTINO').AsString, 8);
      //planilha.cells[iContLinasPlanilha, 7] := sLabelMotivoRetencao;
  ...
  //==================================================================================================================================================================
  stlRelatorio.Clear;
  sComando := 'SELECT MOVIMENTO, ARQUIVO, DATA_POSTAGEM, NUMERO_LOTE FROM ' + objParametrosDeEntrada.TABELA_PROCESSAMENTO
            + ' where mid(CIF_WEB, 1, 2) = 72 '
  ...
  begin
    ...
    sComando := 'SELECT MOVIMENTO, ARQUIVO, DATA_POSTAGEM, NUMERO_LOTE, CEP_STATUS, COUNT(NUMERO_LOTE) AS "QTD", SUM(PESO) AS "PESO" FROM ' + objParametrosDeEntrada.TABELA_PROCESSAMENTO
              + ' where mid(CIF_WEB, 1, 2) = 72 '
              + '   AND ARQUIVO            = "' + __queryMySQL_processamento__.FieldByName('ARQUIVO').AsString + '"'
  ...
  Begin
    ...
    objFuncoesWin.ExecutarArquivoComProgramaDefault(sArquivoREL);
     ...
     //planilha.WorkBooks[1].SaveAs(StringReplace(sArquivoREL, '.txt', '.xls', [rfReplaceAll, rfIgnoreCase]));
